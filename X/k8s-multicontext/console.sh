...
sshpass -p 'vagrant' scp -o StrictHostKeyChecking=no root@192.168.2.$2:/etc/hosts /tmp/$2-hosts
sshpass -p 'vagrant' scp -o StrictHostKeyChecking=no root@192.168.2.$3:/etc/hosts /tmp/$3-hosts
sshpass -p 'vagrant' scp -o StrictHostKeyChecking=no root@192.168.2.$4:/etc/hosts /tmp/$4-hosts
cat /tmp/$2-hosts >> /etc/hosts; cat /tmp/$3-hosts >> /etc/hosts; cat /tmp/$4-hosts >> /etc/hosts

# copy kubeconfig by sshpass
sshpass -p 'vagrant' scp -o StrictHostKeyChecking=no root@192.168.2.$2:/root/.kube/config $va_k8s_cfg-$2
sshpass -p 'vagrant' scp -o StrictHostKeyChecking=no root@192.168.2.$3:/root/.kube/config $va_k8s_cfg-$3
sshpass -p 'vagrant' scp -o StrictHostKeyChecking=no root@192.168.2.$4:/root/.kube/config $va_k8s_cfg-$4
